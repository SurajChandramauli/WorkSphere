...
const AllEmployees = (props) => {
  ...
  useEffect(() => {
    if (props.myMockData && props.myMockData.length > 0) {
      var tempArr = [];
      {
        props.myMockData.map((emp) => tempArr.push(emp));
      }
      setEmployeeMasterData(props.myMockData);
    }
  }, [props.myMockData]);
...
const mapStateToProps = (state) => {
  return {
    myMockData: state.testMockApiDataSetup.myMockData,
...
const mapDispatchToProps = (dispatch) => {
  return {
    getTestMockData: () => dispatch(getTestMockData()),
